...
const VerifyOTP = () => {
  ...
  // Get the email from localStorage when the component is mounted
  useEffect(() => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await fetch("https://oldertech-jatinsingh0s-projects.vercel.app/api/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, otp: otpCode }), // Pass email and OTP
      });
